...
namespace Lab_1
{
   class Scanner
   {
         ...
         foreach (char letter in letters)
         {
            ...
            if (Status == Statuses.Start)
            {
               ...
               else
               {
                  ...
                  else if (Operations.SearchIsExist(letter.ToString()) || letter == '!')
                  {
            ...
            if (Status == Statuses.ReadName)
            {
               ...
               {
                  if ("qwertyuiopasdfghjklzxcvbnmQWERTYUIOPASDFGHJKLZXCVBNM_".Contains(letter) || "1234567890".Contains(letter))
                     name += letter;
                  else
                     Status = Statuses.Error;
               }
            ...
            if (Status == Statuses.ReadComment)
            {
               if (comment == "/")
               {
                  if (letter == '/')
                  {
                     Console.WriteLine("линейный коммент");
                     flagLineComment = true;
                  }
                  else if (letter == '*')
                  {
                     Console.WriteLine("блочный коммент");
      ...
      {
         using (f = new StreamWriter("Tokenы.txt")){
